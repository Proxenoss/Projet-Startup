...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const menuData = [
    ...
    { text: 'À propos', href: 'A propos/A proposfr.html' }//crée les liens vers les pages
  ];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateHorloge() {
        ...
        horloge.textContent = `Heure: ${heures}:${minutes}:${secondes}`;//affiche l'heure
    }

    updateHorloge();
    setInterval(updateHorloge, 1000);//update l'horloge toute les secondes

    let secondesPassees = 0;
    function updateChrono() {
        secondesPassees++;//incremente un compteur  qui augmente toutes les secondes
        chrono.textContent = `Temps passé : ${secondesPassees}s`;//affiche les secondes
    }
    setInterval(updateChrono, 1000);//update le chrono toute les secondes
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    campusContainer.innerHTML = `<h3>Coordonnées de nos campus</h3>`;//creer une nouvelle div avec un titre
